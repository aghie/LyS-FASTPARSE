...
from covington import CovingtonLSTM
...
if __name__ == '__main__':
    ...
    parser.add_option("--kb", type="int", dest="window_b", default=1)
    parser.add_option("--k1", type="int", dest="window_l1", default=3)
    parser.add_option("--k2r", type="int", dest="window_l2r", default = 1)
    parser.add_option("--k2l", type="int", dest="window_l2l", default = 1)  
    parser.add_option("--lr", type="float", dest="learning_rate", default=0.1)
    parser.add_option("--outdir", type="string", dest="output", default="results")
    parser.add_option("--activation", type="string", dest="activation", default="tanh")
    parser.add_option("--optimizer",type="string", dest="optimizer", default="adam")
    ...
    if not options.predictFlag:
        ...
        parser = CovingtonLSTM(words, pos, rels, w2i, options)
        ...
        for epoch in xrange(options.epochs):
            ...
            os.system('perl /home/david.vilares/Software/MaltOptimizer-1.0.3/eval.pl -g ' + options.conll_dev + ' -s ' + devpath  + ' > ' + devpath + '.txt &')
        ...
        os.system('perl /home/david.vilares/Software/MaltOptimizer-1.0.3/eval.pl -g ' + options.conll_test + ' -s ' + tespath  + ' > ' + tespath + '.txt &')
...
#     parser.add_option("--userl", action="store_true", dest="rlMostFlag", default=False)
#     parser.add_option("--predict", action="store_true", dest="predictFlag", default=False)
#     parser.add_option("--cnn-mem", type="int", dest="cnn_mem", default=2000)
# 
#     (options, args) = parser.parse_args()
#     print 'Using external embedding:', options.external_embedding
# 
...
#         for epoch in xrange(options.epochs):
#             print 'Starting epoch', epoch
#             parser.new_Train(options.conll_train)
#             devpath = os.path.join(options.output, 'dev_epoch_' + str(epoch+1) + '.conll')
#             utils.write_conll(devpath, parser.Predict(options.conll_dev))
#             os.system('perl /home/david.vilares/Software/MaltOptimizer-1.0.3/eval.pl -g ' + options.conll_dev + ' -s ' + devpath  + ' > ' + devpath + '.txt &')
#             print 'Finished predicting dev'
